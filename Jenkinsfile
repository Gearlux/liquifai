...
// Generated by: aisland jenkins scaffold --project liquifai
// Source template: aisland/services/jenkins.py (JenkinsService.scaffold_pipelines)
// To regenerate:  aisland jenkins scaffold --project liquifai --force
// =========================================================================
pipeline {
    agent any
                ...
                stage('Black') {
                    steps {
                        script {
                            ...
                            def targets = sh(script: "for d in liquifai tests examples; do if [ -d \"\$d\" ] && find \"\$d\" -name '*.py' | grep -q .; then printf \"%s \" \"\$d\"; fi; done || true", returnStdout: true).trim()
                            if (targets) {
                        ...
                        always {
                            script {
                                if (fileExists('black-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'black-liquifai',
                                        name: 'Black Formatting (Liquifai)',
...
" """
                        ...
                        always {
                            script {
                                if (fileExists('isort-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'isort-liquifai',
                                        name: 'Isort Import Order (Liquifai)',
            ...
            steps {
                script {
                    if (fileExists('tests') && sh(script: "find tests -name '*.py' | grep -q .", returnStatus: true) == 0) {
                        sh "${VENV_BIN}/pytest tests --junitxml=test-report.xml --cov=liquifai --cov-report=xml:coverage.xml --cov-report=term"
                    } else {
                ...
                always {
                    script {
                        ...
                        if (fileExists('coverage.xml')) {
                            recordCoverage(
                                id: 'coverage-liquifai',
                                name: 'Liquifai Coverage',
    ...
    post {
        always {
            echo 'Liquifai Pipeline Complete.'
        }
        success {
            echo 'Liquifai is healthy.'
        }
        failure {
            echo 'Liquifai build failed. Please check linting or test failures.'
        }
    }
}
