...
``recordstream.record.Record`` (a NamedTuple of ``(input, target, metadata)``)
stored in a source's ``__dict__`` got rebuilt as
``Record(input=[input, target, metadata], target=None, metadata={})`` —
...
class _Record(NamedTuple):
    """Mirrors recordstream.record.Record's NamedTuple shape."""
...
def test_deep_flow_preserves_namedtuple_field_layout() -> None:
    ...
    items = [
        _Record(input="A", target=1, metadata={"i": 0}),
        _Record(input="B", target=2, metadata={"i": 1}),
    ]
    holder = _Holder(items)
    deep_flow(holder)

    # Each Record's fields must be unchanged.
    for i, expected in enumerate(items):
        actual = holder.items[i]
        assert isinstance(actual, _Record)
...
def test_deep_flow_namedtuple_directly() -> None:
    ...
    record = _Record(input="X", target=42, metadata={"k": "v"})
    result = deep_flow(record)
    assert isinstance(result, _Record)
